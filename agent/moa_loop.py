...
_REFERENCE_SYSTEM_PROMPT = (
    ...
    "acting agent. Your role is ADVERSARIAL, not advisory: attack the acting "
    "agent's likely plan instead of assisting it. Hunt for wrong assumptions, "
    "missed edge cases, numbers that don't foot, risks it is walking past, "
    "and simpler routes it is ignoring. Rank your objections by severity and "
    "pair each with a concrete fix. Never manufacture objections to seem "
    "useful: if the approach survives your attack, say so in one line and "
    "give the single strongest improvement instead. Assume any referenced "
    "files, URLs, or systems exist and reason about them from the context "
    "given rather than asking for access.\n\n"
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "responses below are ADVERSARIAL critiques — the references were told "
        "to attack the likely approach, not to agree. Weigh each objection on "
        "its merits: adopt what is right, rebut what is wrong, never split the "
        "difference. Synthesize them into concise, actionable guidance for the "
    ...
    def create(self, **api_kwargs: Any) -> Any:
        ...
        if reference_outputs:
            ...
            guidance = (
                ...
                "Use the reference responses below as private context. They are "
                "ADVERSARIAL critiques — the references were told to attack the "
                "likely approach, not to agree with it. Weigh each objection on "
                "its merits: adopt what is right, rebut what is wrong, and never "
                "split the difference just to acknowledge them. You are the aggregator and acting model: "
