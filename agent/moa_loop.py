...
_REFERENCE_SYSTEM_PROMPT = (
    ...
    "acting agent. Sharpen its likely plan: hunt for wrong assumptions, missed "
    "edge cases, numbers that don't foot, and real risks it is walking past, "
    "and pair each with a concrete fix. But be disciplined, not reflexive:\n"
    "- Acknowledge work already done or tested as DONE. Do NOT re-raise "
    "objections about finished or previously resolved steps; critique only what "
    "is NEW or genuinely unaddressed in the latest step.\n"
    "- If the approach is sound, or the task is routine (a lookup, a read, a "
    "single obvious action), say so in one line and stop. Never manufacture "
    "objections to seem useful.\n"
    "- When the plan is progressing, name the single most important next risk "
    "or step instead of re-listing everything not yet built.\n"
    "Assume any referenced files, URLs, or systems exist and reason about them "
    "from the context given rather than asking for access.\n\n"
    "Respond with your critique directly — no praise, no padding, no "
    "disclaimers about tools or access. Your response is private guidance "
    "handed to the aggregator, not an answer shown to the user. NEVER claim to "
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        "You are the aggregator in a Mixture of Agents process. The reference "
        "responses below are a critical review of the approach. Weigh each point "
        "on its merits: adopt what is right, rebut what is wrong, never split the "
        "difference. Do NOT narrate the review or announce what the reviewer said — "
        "fold the useful parts into your own thinking silently. Synthesize them "
        "into concise, actionable guidance for the "
        "main Hermes agent. Focus on next steps, tool-use strategy, risks, and "
        "any disagreements. Do not answer the user directly unless that is all "
        "that is needed; produce context the main agent should use in its normal loop.\n\n"
    ...
    def create(self, **api_kwargs: Any) -> Any:
        ...
        if reference_outputs and not successful_outputs:
            ...
            guidance = (
                ...
                "Use the reference responses below as private context. They are a "
                "critical review of the approach. Weigh each point on its merits: "
                "adopt what is right, rebut what is wrong, and never split the "
                "difference just to acknowledge them. Do NOT announce or defer to the "
                "review out loud (no \"good critique from the ref, let me redo\" "
                "narration) — just act on the useful parts. You are the aggregator and acting model: "
