...
One config block in ``~/.hermes/config.yaml`` controls everything::
    ...
    proprioception:
      ...
      heartbeat: delta             # off | delta | always
      min_interval_seconds: 60     # per-session floor between non-critical emissions
      cache_ttl_seconds: 20        # process-wide snapshot cache
...
unreadable config all resolve to ``enabled: false``.
"""
...
def get_settings() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("proprioception: config read failed; staying disabled", exc_info=True)
