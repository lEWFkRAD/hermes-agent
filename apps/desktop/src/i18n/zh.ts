...
export const zh: Translations = {
  ...
  settings: {
    ...
    localModels: {
      ...
      installDetail:
        '下载 llama.cpp 推理引擎（几百 MB）。模型和聊天会留在本机；只有你查看并明确提交的基准测试报告才会离开本机。',
      installAction: '安装运行时',
      installing: '正在安装运行时…',
      installFailed: '运行时安装失败',
      quickstartTitle: '在本机运行模型',
      quickstartDetail: (model, size) =>
        `一键完成所有设置：本地引擎、${model}（需下载 ${size}），并设为新会话的默认模型。模型和聊天会留在本机，除非你明确提交基准测试报告。`,
      ...
      placementDiskBackedTip: size =>
        `${size} 查找表会按需从磁盘读取。这不是系统内存溢出，但模型并非完全驻留在 GPU 上。`,
      loadingPill: '加载中…',
      ...
      pillUsesRam: '使用系统内存',
      pillDiskBacked: '磁盘后备查找表',
      pillDiskBackedTip: size => `${size} 查找表会按需从磁盘读取。这与常规系统内存溢出是分开的。`,
      pillTooBig: '超出本机内存',
      browseTitle: '发现更多模型',
      browseHint:
        '选择发布者提供的 GGUF 量化版本。Hermes 会在使用前检查完成的文件，包括大型查找表是否保持磁盘后备；不会重写模型。',
      browseFitEstimate: label => `估算：${label}`,
      browseEstimateTip:
        '这只基于下载大小。下载后，Hermes 会读取 GGUF 头部并说明大型查找表是磁盘后备还是使用常规内存。',
      ...
      lookupResident: '查找表使用常规内存',
      lookupResidentTip: size => `${size} 查找表属于模型的常规内存占用；它不是磁盘后备，也不会单独卸载。`,
      lookupUpdateRequired: '磁盘后备查找表需要更新',
      lookupUpdateRequiredTip: engine =>
        `大型查找表只有在 llama.cpp ${engine} 或更高版本中才会保持磁盘后备。使用此模型前请更新本地引擎。`,
      lookupUnknown: '无法检查查找表',
      lookupUnknownTip: 'Hermes 无法读取此 GGUF 标头，因此无法确认查找表将如何放置。',
      browseDownloadConfirm: size => `此发布者提供的 GGUF 为 ${size}，可能不适合此设备。仍要下载吗？`,
      sideloadButton: '添加模型文件',
      sideloadTitle: '选择 GGUF 模型文件',
      sideloadDone: '已添加 {name}。',
      sideloadAlreadyPresent: '已在你的库中。',
      benchmarkTitle: '分享本地基准测试',
      benchmarkSubtitle: '运行简短的本地测试，查看报告后再选择是否发送。绝不会自动提交。',
      benchmarkRunTitle: '运行基准测试',
      benchmarkRun: '运行基准测试',
      benchmarkRunning: '正在运行基准测试…',
      benchmarkRunDetail: model => `用简短的本地生成测试测量 ${model}。`,
      benchmarkServerNeeded: '请先启动本地服务器再运行基准测试。',
      benchmarkReportModelTitle: '模型',
      benchmarkReportRuntime: '运行时',
      benchmarkReportHardwareTitle: '硬件',
      benchmarkReportPlacementTitle: '内存放置',
      benchmarkReportResultsTitle: '基准测试结果',
      benchmarkReportWorkloadTitle: '测试负载',
      benchmarkReportMetaTitle: '报告',
      benchmarkReportModel: (family, quant, weights, lookup) =>
        `${family} · ${quant} · 权重 ${weights} · 查找表 ${lookup}`,
      benchmarkReportConfig: (engine, context, slots, kvCache, speculation) =>
        `${engine} · ${context} 上下文 · ${slots} 个槽位 · ${kvCache} KV · ${speculation}`,
      benchmarkReportHardware: (deviceMemory, systemMemory, unifiedMemory) =>
        `${deviceMemory} 设备内存 · ${systemMemory} 系统内存${unifiedMemory ? ' · 统一内存' : ''}`,
      benchmarkReportPlacement: (lookup, ordinaryMemorySpill) =>
        `${lookup} · ${ordinaryMemorySpill ? '使用系统内存' : '没有常规内存溢出'}`,
      ...
      benchmarkReportWorkload: (request, promptTokens, completionTokens) =>
        `${request} · 提示 ${promptTokens.toLocaleString()} 个 token · 生成 ${completionTokens.toLocaleString()} 个 token`,
      benchmarkReportMeta: (packageId, createdAt, schemaVersion) =>
        `一次性报告 ${packageId} · 创建于 ${createdAt} · ${schemaVersion}`,
      ...
      benchmarkNoLookup: '无查找表',
      benchmarkReviewTitle: '分享前请确认',
      benchmarkPrivacy:
        '仅包含一次性报告 ID 和创建时间；模型系列/量化、权重和查找表大小/放置；运行时构建/设置；设备和系统内存容量；以及基准测试时长、词元计数和速率。绝不包含提示文本、生成文本、聊天、文件、路径、主机名、账号、别名或持久安装 ID。',
      benchmarkSubmit: '提交报告',
      benchmarkSubmitting: '正在提交报告…',
      benchmarkSubmitTitle: '提交这份基准测试报告？',
      benchmarkSubmitDescription:
        '仅发送你已查看的一次性报告 ID 和创建时间；模型系列/量化、权重和查找表大小/放置；运行时构建/设置；设备和系统内存容量；以及基准测试时长、词元计数和速率。绝不包含提示文本、生成文本、聊天、文件、路径、主机名、账号、别名或持久安装 ID。今后的报告仍需要你的确认。',
      benchmarkSubmitted: '基准测试报告已提交。',
      benchmarkFailed: '无法运行或提交基准测试',
      benchmarkSharingEnabled: '手动分享已启用',
      benchmarkStopSharing: '关闭提交同意',
      benchmarkConsentStopped: '基准测试提交同意已关闭。',
